...
```
Hago fuzzing también en el puerto 80, pero no encuentro nada, la web es muy simple y solo nos ofrece la descarga
de una apk "catchv1.0.apk"
...
Otra cosa interesante al hacer con un apk es abrir el codigo con herramientas como jadx-gui (desensamblador)
donde se leakea este subdominio -> ```https://status.catch.htb/```

Cuando entro en lets_chat me sale un panel de login (puerto 3000), si busco en google como usar el token 
```lets chat token``` la primera entrada es un repo de github que habla de una api. (aunque no exite la ruta /api)
...
Si enumeramos el sistema, varias veces de ha corrido  "_ /bin/bash /root/check.sh" pero no podemos acceder a el
...
# Cleanup #
cleanup() {
	rm -rf /root/mdm/process_bin;rm -rf "/opt/mdm/apk_bin/*" "/root/mdm/apk_bin/*";rm -rf $(ls -A /opt/mdm | grep -v apk_bin | grep -v verify.sh)
	}
...
```
Si ve que en ./res/values/strings.xml pone el nombre de la apk y tiene la string Catch dentro, te 
hace el comando de bash de crear una carpeta con dicho nombre
...
Si en el campo config[mail_address] le pones varaibles de entorno del tipo ${DB_HOST} te las filtra.
He replicado el exploit que he encontrado reprogramandolo yo mismo.
