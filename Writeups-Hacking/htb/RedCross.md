...
/POST a intra.redcross.htb/pages/actions.php  Cookie: PHPSESSID=isf5g87laf3r3glg34arsoj354 user=admin&pass=admin&action=login. Al poner creds que no valen dice "Wrong Data"

Tambien ha un boton "go login" que hace un POST a /?page=login con la data action=go+login.

Podriamos intentar fuzzing e inyecciones pero parece que los tiros novan por ahí porque abajo pone:
...
```
Basicamente dice que el campo del mensaje es vulnerable a un XSS con el payload clásico:
...
Con esta extensión del navegador puedes moverte por toda la página arrastrando las cookies (son diferentes 
pero porque la cague dandole a cerrar sesion y tuve que secuestrarlas otra vez)

En la cookie hay un campo que ponía "DOMAIN":"admin", y en los mensajes hablan de un panel de asministración, 
por lo que debería existir un admin.redcross.htb. Si ponemos este subdominio en **/etc/hosts** nos resuelve a esta página.

En network acces hay una sección llamada "Allow IP Adress", que sirve para abrir el firewall. Pongo la mía.

Si volvemos a hacer un escaneo de puertos, vemos que están abiertos también los 21,1025,5432:
- Nmap nos dice que -> 21 es ftp, 1025 NFS y 5432 postgresSQL.

Cuando le das a allow an ip hay una peticion a admin.redcross.htb/pages/actions.php ,ip=10.10.14.12&action=Allow+IP

La seccion adduser si le pongo "cucuxii" -> cucuxii : 4QSjFXtt, hacer un ssh con este usaurio nos da un shell muy limitada.
Podemos acceder a muy pocos archivos, entre ellos /home/public/src/iptctl.c Un script en c, como no sabemos que 
hace nos lo copiamos en neustro sistema.

Si seguimos con el tema del firewall, podriamos intentar un SO command inyection (ya que probablmente esta ip
se le pase al comando iptables o cualquier otro)
...
En esa misma ruta hay un users.txt con las creds:
"$dbconn = pg_connect("host=127.0.0.1 dbname=unix user=unixnss password=fios@ew023xnw");"

Pero por ahora no valen para nada. Subimos nuestro script de reconocimiento
...
```
Si le hacemos un md5sum a ambos binarios (el que hemos traido mas el original) vemos que los hashes son iguales
por lo que no ha sufrido cambios en el camino.
...
```
Error de segmentación, es decir buffer overflow. El problema lo causa fgets en la función de interactive ya
que intenta meter algo en el buffer que es mas grande que él.
...
Al no haber PIE, las direcciones en la PLT de execvp y setuid están estáticas.
Querriamos manipularlas para que salga **execvp("bash",0)** y **setuid(1)**
```console
...
```
¿Como le pasamos los argumentos a las funciones luego? Como estamos en 64 bits funciona tal que una función lee
los argumentos guardados en registros en este orden de prioridad -> rdi rsi rdx r8 y r9
Esos registros primero hay que abrirlos y luego cargar valores dentro.
```console
...
```
He creado el exploit en la máquina vicima para evitar problemas y como corre co python he usado las liberias
más básicas como struct en vez de tirar de pwntools.
