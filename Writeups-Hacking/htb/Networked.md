...
foreach (scandir($path) as $file) {   // Por cada archivo...
  if (in_array($file, $ignored)) continue;  // Salvo '.' '..' e 'index'
...
function check_ip($prefix,$filename) {
  ...
  if (!(filter_var($prefix, FILTER_VALIDATE_IP))) {
    $ret = false; $msg = "4tt4ck on file ".$filename.": prefix is not a valid ip ";
  } else { $msg = $filename; }
